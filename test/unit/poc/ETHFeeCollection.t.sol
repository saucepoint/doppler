...
// Use a mock migrator since UniswapV2Migrator does not accept native eth
...
contract ETHFeeCollection is Test, Deployers {
    ...
    function test_migrate_v4_native() public {
        ...
        uint256 airlockBalanceNumeraireBefore = currency0.balanceOf(address(airlock));
        ...
        uint256 airlockBalanceNumeraireAfter = currency0.balanceOf(address(airlock));
        uint256 airlockBalanceAssetAfter = IERC20(asset).balanceOf(address(airlock));

        // reverts since fee distribution is incorrect and only pays out ERC20 and not native ether
